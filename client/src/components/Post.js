...
const Post = ({bookmarkedData,setBookmarkedData,setPostData,postData}) => {
    ...
    useEffect(()=>{
        if(bookmarkedData?.includes(postData.content)) setBookmarkToggle(true);
    ...
    const handleBookmarkToggle = (e)=>{
        ...
        if(bookmarkToggle){
            ...
            axios.post('http://localhost:8080/post/delete/bookmark',{
            ...
            axios.post('http://localhost:8080/post/insert/bookmark',{
                email: userInfo.email,
                content: postData.content
            })
            getBookmarkedData();
    ...
    const handleAwardClick = (e,i,user_name,address) => {
        e.preventDefault();
        navigate('/buynft/'+i+`?user_name=${user_name}&address=${address}`);
    }
        ...
        <div className='post'>
            ...
            <div className='post__contents'>
                ...
                <div className='post__contents__interact'>
                            ...
                            awardToggle&&<div className='award-nft'>
                                ...
                                <div className='award-nft__container'>
                                {
                                    ...
                                    NFTdata.map((el,i)=>{
                                        return <div
                                        onClick={(e)=>{
                                            handleAwardClick(e,el.id,postData?.user_name,postData?.address);
                                        }}
                                        key = {el.id} className='award-nft__award' >
                    ...
                    className='post__contents__interact__item green-hover'> 
                        {!bookmarkToggle&&<svg viewBox="0 0 24 24" aria-hidden="true" fill='#f7f9f9'><g><path d="M4 4.5C4 3.12 5.119 2 6.5 2h11C18.881 2 20 3.12 20 4.5v18.44l-8-5.71-8 5.71V4.5zM6.5 4c-.276 0-.5.22-.5.5v14.56l6-4.29 6 4.29V4.5c0-.28-.224-.5-.5-.5h-11z"></path></g></svg>}
                        {bookmarkToggle&&<svg viewBox="0 0 24 24" aria-hidden="true" ><g><path d="M4 4.5C4 3.12 5.119 2 6.5 2h11C18.881 2 20 3.12 20 4.5v18.44l-8-5.71-8 5.71V4.5z"></path></g></svg>}
                        <span className='green-hover'>{bookmarkToggle?'Bookmarked':'Bookmark'}</span>
                    </div>}
                </div>
            </div>
        </div>
